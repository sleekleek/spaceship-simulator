...
class GLRenderer():
    ...
    def cycle_cameras(self):
        ...
        if not self.scene.cameras:
            return None
        self.current_cam_index = (
            self.current_cam_index + 1) % len(self.scene.cameras)
    ...
    def apply_material(self, mat):
        ...
        # evaluate once the mat properties, and cache the values in a glDisplayList.
        if not hasattr(mat, "gl_mat"):
            diffuse = numpy.array(mat.properties.get(
                "diffuse", [0.8, 0.8, 0.8, 1.0]))
            specular = numpy.array(mat.properties.get(
                "specular", [0., 0., 0., 1.0]))
            ambient = numpy.array(mat.properties.get(
                "ambient", [0.2, 0.2, 0.2, 1.0]))
            emissive = numpy.array(mat.properties.get(
                "emissive", [0., 0., 0., 1.0]))
    ...
    def recursive_render(self, node):
        ...
        for mesh in node.meshes:
            ...
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mesh.gl["triangles"])
            glDrawElements(GL_TRIANGLES, len(mesh.faces)
